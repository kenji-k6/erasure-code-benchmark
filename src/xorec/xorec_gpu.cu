...
__global__ void xorec_gpu_encode_kernel(
  ...
  // Always only one GPU block!!
  // => Num threads = MAX_THREADS_PER_BLOCK
  // One Data block is directly mapped to one warp
  if (gridDim.x != 1) {
  ...
  for (uint32_t i = warp_idx; i < num_data_blocks; i += num_warps) {
    const uint64_t * XOR_RESTRICT data_block = reinterpret_cast<const uint64_t*>(data_buffer + i * block_size);
    uint64_t * XOR_RESTRICT parity_block = reinterpret_cast<uint64_t*>(parity_buffer + (i%num_parity_blocks) * block_size);
