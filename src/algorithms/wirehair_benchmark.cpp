...
WirehairBenchmarkGPU::WirehairBenchmarkGPU(const BenchmarkConfig& config) noexcept : WirehairBenchmark(config) {}

int WirehairBenchmarkGPU::setup() noexcept {
  if (wirehair_init()) {
    std::cerr << "Wirehair: Initialization failed.\n";
    return -1;
  }

  cudaError_t err = aligned_cudaMallocManaged(reinterpret_cast<void**>(&original_buffer_), block_size_ * num_original_blocks_, ALIGNMENT_BYTES, cudaMemAttachHost);
  if (err != cudaSuccess) {
    std::cerr << "Wirehair: Failed to allocate data buffer.\n";
    return -1;
  }

  cudaError_t err = aligned_cudaMallocManaged(reinterpret_cast<void**>(&decode_buffer_), block_size_ * num_original_blocks_, ALIGNMENT_BYTES, cudaMemAttachHost);
  if (err != cudaSuccess) {
    std::cerr << "Wirehair: Failed to allocate decode buffer.\n";
    return -1;
  }
  encode_buffer_ = static_cast<uint8_t*>(aligned_alloc(ALIGNMENT_BYTES, block_size_ * (num_original_blocks_ + num_recovery_blocks_)));
...
void WirehairBenchmarkGPU::teardown() noexcept {
  if (original_buffer_) aligned_cudaFree(original_buffer_);
  if (encode_buffer_) free(encode_buffer_);
  if (decode_buffer_) aligned_cudaFree(decode_buffer_);
  if (encoder_) wirehair_free(encoder_);
  if (decoder_) wirehair_free(decoder_);
}

void WirehairBenchmarkGPU::make_memory_cold() noexcept {
  touch_memory(original_buffer_, block_size_ * num_original_blocks_);
  touch_memory(decode_buffer_, block_size_ * num_original_blocks_);
}