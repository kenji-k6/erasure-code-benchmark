...
XorecBenchmarkAVX::XorecBenchmarkAVX(const BenchmarkConfig& config) noexcept : ECBenchmark(config) {
  ...
  if (!m_data_buffer || !m_parity_buffer || !m_block_bitmap) throw_error("XorecBenchmarkAVX: Failed to allocate memory.");

  memset(m_block_bitmap, 1, m_num_chunks * m_blks_per_chunk);
  for (unsigned i = 0; i < m_size_msg/m_size_blk; ++i) {
    if (write_validation_pattern(i, m_data_buffer+i*m_size_blk, m_size_blk)) throw_error("XorecBenchmarkAVX: Failed to write validation pattern");
  }
}

XorecBenchmarkAVX::~XorecBenchmarkAVX() noexcept {
...
int XorecBenchmarkAVX::encode() noexcept {
  ...
  for (unsigned i = 0; i < m_num_chunks; ++i) {
    ...
    if (xorec_encode_avx(data_buf, parity_buf, m_size_blk, m_data_blks_per_chunk, m_parity_blks_per_chunk) != XorecResult::Success) {
...
int XorecBenchmarkAVX::decode() noexcept {
  ...
  for (unsigned c = 0; c < m_num_chunks; ++c) {
    ...
    XorecResult res = xorec_decode_avx(data_buf, parity_buf, m_size_blk, m_data_blks_per_chunk, m_parity_blks_per_chunk, bitmap);
...
void XorecBenchmarkAVX::simulate_data_loss() noexcept {
  for (unsigned c = 0; c < m_num_chunks; ++c) {
    uint8_t* data_buf = m_data_buffer + c * m_size_data_submsg;
    uint8_t* parity_buf = m_parity_buffer + c * m_size_parity_submsg;
    uint8_t* bitmap = m_block_bitmap + c * m_blks_per_chunk;
